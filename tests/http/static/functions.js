...
function showList(message = "", list = []) {
  return new Promise((resolve) => {
    if (list.length === 0) {
      resolve(null);
      return;
    ...
    select.addEventListener('dblclick', (event) => {
      ...
      if (clickedElement.tagName === 'OPTION') {
        cleanup(clickedElement.value);
      } else if (clickedElement.tagName === 'SELECT' && clickedElement.selectedIndex !== -1) {
        cleanup(clickedElement.options[clickedElement.selectedIndex].value);
      }
    });
    ...
    buttonOk.addEventListener('click', () => cleanup(select.value));
    buttonCancel.addEventListener('click', () => cleanup(null));
